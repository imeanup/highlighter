    ...
    chrome.runtime.getBackgroundPage(function(b) {
        ! function(c) {
            ...
            var k, l, m = $("#switcher").attr("data-on", c.getActiveStatus() ? "true" : "false").on("click", function() {
                m.attr("data-on", "true" == m.attr("data-on") ? "false" : "true"), clearTimeout(k), k = setTimeout(function() {
                    c.setActiveStatus("true" == m.attr("data-on") ? !0 : !1)
                }, 100)
            });
            j.on("keyup change paste", d), $("#support-link").click(function() {
                chrome.tabs.create({
                    url: "https://www.patreon.com/alexius_lee"
                })
            }), /[\s\r\n]/.test(j.val()[j.val().length - 1]) || "" == $.trim(j.val()) || j.val(j.val() + " "), setTimeout(function() {
                j[0].selectionStart = j[0].selectionEnd = j[0].value.length, $("#support-link")[0].scrollIntoView()
            }, 200)
        }(b.background)
    })
}(this, $(this));